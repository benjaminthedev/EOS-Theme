...
function us_load_metabox_header_settings( $header_settings ) {
	...
	if ( is_singular( array_merge( array( 'post', 'page', 'us_portfolio', 'product' ), $supported_custom_post_types ) ) OR ( is_404() AND $page_404 = get_page_by_path( 'error-404' ) ) ) {
		if ( is_singular() ) {
			$postID = get_the_ID();
		} elseif ( is_404() ) {
			$postID = $page_404->ID;
	...
	foreach ( $layout[$place] as $elm ) {
		if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
			...
			if ( isset( $data[$elm] ) ) {
				...
				if ( us_arr_path( $data[$elm], 'design_options.hide_for_not-sticky', FALSE ) ) {
